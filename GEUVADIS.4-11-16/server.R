...
top3pc <- read.csv("GV_4-11-16_top3.csv", header=TRUE, row.names=1)
# for "Population"
...
unk <- which(grepl("unknown", rownames(top3pc)))
# for "Performer"
...
shinyServer(function(input, output) {
    ...
    #distdiff = luadd - luscd
        ...
        if(colorby == "Population"){
            ...
            cols[unk] <- unkcolpop
            output$distPlot <- renderPlot({
                legend("center", c("TSI", "FIN", "CEU", "YRI", "GBR", "Unknown"), fill=c(pop_colors[1:5],'gray'), bty = "n", pt.cex = 1, cex=1.5)
            })
        }
