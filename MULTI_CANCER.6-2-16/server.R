...
top3pc <- read.csv("4_cancers.eigen_vectors.csv", header=TRUE, row.names=1)

# for "Cancer Type"
lusc <- which(grepl("LUSC", rownames(top3pc)))
luad <- which(grepl("LUAD", rownames(top3pc)))
aml_target <- which(grepl("AML", rownames(top3pc)))
laml_tcga <- which(grepl("LAML", rownames(top3pc)))
...
#luad    <- which(grepl("LUAD", rownames(top3pc)))
#aml_target <- which(grepl("AML", rownames(top3pc)))
#yaml_tcga <- which(grepl("LAML", rownames(top3pc)))
...
## # colors for the 5(6) populations (+ unknown)
## pop_colors <- col.wheel(6)
## pop_colors[6] <- 'gray'
## #pop_colors <- vector()
...
## unkcolpop = "gray"<- pop_colors[6]

## # colors for the 7(8) performers (+ unknown)
## perf_colors <- col.wheel(8)
## perf_colors[8] <- 'gray'
...
laml_tcga <- which(grepl("LAML", rownames(top3pc)))
        ...
        if(colorby == "Cancer_Type"){
            cols[lusc] <- luscCol
            cols[luad] <- luadCol
            cols[aml_target] <- aml_targetCol
            cols[laml_tcga] <- laml_tcgaCol
            output$distPlot <- renderPlot({
                legend("center", c("LUSC", "LUAD", "AML(TARGET)", "LAML(TCGA)"), fill=c(luscCol,luadCol,aml_targetCol,laml_tcgaCol), bty = "n", pt.cex = 1, cex=1.5)
            })
        }
